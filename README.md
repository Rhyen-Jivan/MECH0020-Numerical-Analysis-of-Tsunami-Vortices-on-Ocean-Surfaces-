...
2. `Scripts/Infrastructure/PathSetup.m`
3. `Scripts/Infrastructure/Dispatchers/RunDispatcher.m`
4. `Scripts/Infrastructure/Dispatchers/ModeDispatcher.m`
5. `Scripts/Modes/`
6. active method contract files in `Scripts/Methods/`
7. `Scripts/Infrastructure/Validation/` for current rerun/smoke/plotting helpers

Important architectural rules:

- The canonical runtime path is UI-driven.
- `Tsunami_Vorticity_Emulator.m` is active compatibility support, not the primary orchestration owner.
- `run_simulation_with_method.m` is a supported shim, not the main execution path.
- `FiniteDifferenceMethod.m` is the active FD source of truth.
- `Scripts/Legacy/` is excluded from the active runtime path by `PathSetup.pruned_genpath()`.
- Generated outputs should not be mixed back into active source trees.
